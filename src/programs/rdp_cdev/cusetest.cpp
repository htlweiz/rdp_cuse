...
int main(int argc, char** argv) {
  ...
  const std::string major_key="--major";
  const std::string minor_key="--minor";
  std::string last_key="";

  for(int i=1;i < argc; i++) {
    std::clog << "RDP_CUSE: LOG: Managing arg: " << argv[i] << std::endl;
    if(last_key=="") {
      last_key=argv[i];
      ...
      now=atoi(argv[i]);
      last_key="";
    } else if(last_key==major_key) {
      dev_major = atoi(argv[i]);
      last_key="";
    } else if(last_key==minor_key) {
      dev_minor = atoi(argv[i]);
      last_key="";
    } else {
      std::clog << "RDP_CUSE: ERROR: Unknown key " << last_key << std::endl;
    }
  }
  
  std::clog << "RDP_CUSE: LOG: Starting rdp cuse with values from " << values << " and an unix time of: " << now << std::endl;
  rdp::RdpValueStore local_store = rdp::RdpValueFactory::create(now,values.c_str());
  std::clog << "RDP_CUSE: LOG: Managed to read " << local_store.get_value_count() << " values from " << values << std::endl;
  the_store = &local_store;

  const char* cusearg[] = {"test", "-f", "-d"};
  char devargbuffer[200];
  sprintf(devargbuffer, "DEVNAME=rdp_cdev%d", dev_minor);
  const char* devarg[]  = { devargbuffer };
  std::clog << "RDP_CUSE: LOG: devname: " << devargbuffer << std::endl;
  struct cuse_info ci;
  ...
  std::clog << "RDP_CUSE: LOG: Starting the cuse_lowlevel_main." << std::endl;
  auto return_value = cuse_lowlevel_main(3, (char**) &cusearg, &ci, &cusetest_clop, NULL);
  std::clog << "RDP_CUSE: LOG: cuse_lowlevel_main stopped with: " << return_value << std::endl;
  return return_value;
}
